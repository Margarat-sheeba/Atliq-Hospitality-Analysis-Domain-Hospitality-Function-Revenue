# Atliq Hospitality Analysis Domain Hospitality Function Revenue

🚀 **End-to-End Data Analytics Project using Power BI & Python**

## 📌 Project Overview

AtliQ Grands is a chain of five-star hotels operating across India, with over 20 years of experience in the hospitality industry. Recently, the company has been losing market share and revenue in the luxury and business hotel segment due to increased competition and ineffective management decisions. To address this, the managing director decided to adopt Business and Data Intelligence strategies to regain their competitive position. However, the company lacks an in-house data analytics team to generate these insights.

As a result, the revenue management team chose to hire a third-party service provider to analyze their historical data and deliver actionable insights.

## 👩‍💻 About Me

Hi, I’m **Margarat Sheeba**, a passionate Data Analyst skilled in transforming raw data into meaningful business insights.
---
Click below for Powerpoint presentation, Interactive dashboard link and Dashaboard Video

## 📊 Project Resources

- 📄 Presentation: [Download PPT](./Atliq%20Hospitality%20Analysis%20PPT.pptx)
- 📁 Dataset: [Download Data](./datasets/)
- 🧾 [Metadata](meta_data_hospitality.txt)
- 📊 [Metrics List](./metrics_list.xlsx)
- 📈 [Power BI Dashboard (.pbix)](./Hospitality%20Data%20Analysis.pbix)
- 🐍 [Python](./Hospitality%20Analysis.ipynb)

## 🎯 Business Problem

Atliq Hotels faced challenges in:
- Declining occupancy rates  
- High cancellation losses  
- Inconsistent customer ratings  
- Inefficient pricing strategies  

This project aims to solve these problems using data analytics.

---

## 📊 Key KPIs Used

- **RevPAR (Revenue per Available Room)**  
- **ADR (Average Daily Rate)**  
- **Occupancy %**  
- **DSRN, DBRN, DURN**  
- **Realization Rate**

---

## 🛠️ Tools & Technologies

- **Power BI** – Dashboard & Visualization  
- **Python (Pandas, NumPy)** – Data Cleaning & Analysis  
- **Excel** – Initial data exploration  
- **Data Modeling** – Star Schema  

---

## 📈 Dashboard Features

- Interactive filters (City, Room Type, Week)
- Revenue & Occupancy trends
- Booking platform analysis
- Hotel-wise performance comparison

---

## 🔍 Key Insights

- Mumbai contributes ~39% of total revenue  
- Delhi leads in occupancy and customer ratings  
- Week 32 shows highest RevPAR (8.31K)  
- Weekends have higher occupancy, weekdays generate ~69% revenue  
- Online bookings outperform offline channels  
- Cancellation losses ~200M impacting revenue  

---

## 💡 Recommendations

- Improve customer experience (staff training, service quality)  
- Optimize underperforming hotels (Atliq Seasons)  
- Expand high-performing markets (Mumbai)  
- Reduce cancellations with better booking strategies  

---

## 🎥 Project Demo

👉 [Video Presentation](https://www.linkedin.com/posts/margarat-sheeba-485885b3_dataanalytics-powerbi-python-activity-7439917051588091904-kN1Q?utm_source=share&utm_medium=member_desktop&rcm=ACoAABgn_GUBs0wkNzEjXmHx0SMwswU973w-z3E)

---

## 📊 Power BI Dashboard

👉 [View Dashboard](https://app.powerbi.com/view?r=eyJrIjoiZmQ1NzM4ZjctMDI3Zi00OTM1LWI4NDItMDcxMGJlYjhiZTI0IiwidCI6ImM2ZTU0OWIzLTVmNDUtNDAzMi1hYWU5LWQ0MjQ0ZGM1YjJjNCJ9)

---
# Data Model
<img width="1215" height="705" alt="image" src="https://github.com/user-attachments/assets/13f0d4eb-a4bc-4c80-8e93-003a7ec7cf74" />

# Dashboard
<img width="1244" height="1962" alt="image" src="https://github.com/user-attachments/assets/314b3ae2-b57c-4037-b4e4-fa027aa1ba8e" />
<img width="1219" height="2102" alt="image" src="https://github.com/user-attachments/assets/37271fba-583e-46c9-8c31-bdcb49a3ee69" />
<img width="1293" height="2006" alt="image" src="https://github.com/user-attachments/assets/310e09dd-e2ae-4c1c-9e04-08365c2a640e" />
<img width="1171" height="1898" alt="image" src="https://github.com/user-attachments/assets/58c63696-9d62-40f0-9189-dfcdb4b8b2dd" />

---

## 🙏 Acknowledgment

Special thanks to:
- **Codebasics**
- **Hemanand Vadivel**
- **Dhaval Patel**

for their guidance and learning resources.

---

## ⭐ What I Learned

- End-to-end data analysis workflow  
- Building interactive dashboards  
- Business storytelling with data  
- Converting insights into decisions  

---

## 📬 Connect With Me

- LinkedIn: www.linkedin.com/in/margarat-sheeba-485885b3 
- Email: sheebapearll@gmail.com  

---

⭐ *If you found this project useful, feel free to star the repo!*
